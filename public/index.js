...
export function volumeSlider(audioElement) {
...
function audioSlider(audioElement) { // This
...
export function updateTitleAndArtist(title, artist) {
...
export function playNext(override) {
  const checkStatus = document.getElementById("autoplay");
...
export function populatePlayListContentTable() {
  // autoplay on
  console.log("hello");
  const checkStatus = document.getElementById("autoplay");
  checkStatus.checked = true;
...
function addRow(table, values, i, playlistRowsAdd, rowPointer, j, isBranch, tempRow) {
  ...
  // var row = table.insertRow();
  // add a row
  playlistRowsAdd.push(row);// Playlist
  // set initial playlist to main one
  if (j != 0 || isBranch) { // Playlist
    row.style.display = 'none';
  }
  var nameSpot = row.insertCell(0);
  var speakerSpot = row.insertCell(1);
...
window.populatePlayListContentTable = populatePlayListContentTable;
